...
export async function POST(req: NextRequest) {
  ...
  if (fechasAChequear.length > 0 && horasAfectadas.length > 0) {
    // 1. Reservas confirmadas o pendientes en ese slot
    // Si el bloqueo es de una sección específica: solo conflictan reservas de esa
    // misma sección o de la cancha completa (seccion_id null). Si el bloqueo es de
    // toda la cancha (seccion_id null): conflicta cualquier reserva, de cualquier sección.
    ...
    const conflictosFuturos = (conflictos ?? []).filter(r => {
      ...
      if (!seccion_id) return true; // bloqueando toda la cancha: cualquier reserva es conflicto
