  ...
  useEffect(() => {
    ...
    if (token) {
      fetch('/api/auth/me', { headers: { Authorization: `Bearer ${token}` } })
        ...
        .catch(() => { setEsInvitado(true); setAuthChecked(true); });
    } else {
      setEsInvitado(true);
      setAuthChecked(true);
    }
  ...
  const validarDatos = (): boolean => {
    if (esInvitado) {
      ...
      if (telefonoWa && !/^9\d{8}$/.test(telefonoWa)) {
        setTelefonoWaError('Número inválido (9 dígitos, empieza en 9)'); ok = false;
      } else { setTelefonoWaError(''); }
      return ok;
    } else {
      if (!telefonoRegistrado.trim()) {
        setTelefonoError('Ingresa tu número para devoluciones'); return false;
      }
      if (!/^9\d{8}$/.test(telefonoRegistrado.trim())) {
        setTelefonoError('Número inválido (9 dígitos, empieza en 9)'); return false;
      }
      setTelefonoError('');
      return true;
    }
  ...
  const handleEnviar = async () => {
    ...
    if (soloEfectivo) {
      if (esInvitado) {
        ...
        setEmailError('');
        if (telefonoWa && !/^9\d{8}$/.test(telefonoWa)) { setTelefonoWaError('Número inválido (9 dígitos, empieza con 9)'); return; }
        setTelefonoWaError('');
      }
      setEnviando(true);
      const res = await apiCrearReserva({
        ...
        ...(esInvitado && emailInvitado ? { emailInvitado, telefonoInvitado: telefonoInvitado || '', metodoDevolucion: 'yape', ...(telefonoWa ? { whatsappInvitado: telefonoWa } : {}) } : {}),
      });
      if (res.error) {
        setEnviando(false);
        setSubmitError(res.error.includes('ya fue reservado') ? 'Este horario ya fue reservado por otro usuario. Vuelve y elige otro horario.' : 'No se pudo crear la reserva. Intenta de nuevo.');
        return;
      }
      limpiarInicioBloqueo(canchaId, fecha, hora);
      fetch('/api/bloqueos', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ canchaId, fecha, hora, horas }) });
    ...
    const result = await apiCrearReserva({
      canchaId, canchaNombre: cancha.name, fecha, hora, horas,
      precio: total, precioOriginal: precioRaw,
      cuponId: cuponSeleccionado, metodoPago: metodo,
      ...
      ...(!esInvitado ? {
        telefonoDevolucion: telefonoRegistrado ? `${telefonoRegistrado} (${metodo})` : '',
    ...
    if (result.error) {
      setEnviando(false);
      setSubmitError(result.error.includes('ya fue reservado') ? 'Este horario ya fue reservado por otro usuario. Vuelve y elige otro horario.' : 'No se pudo crear la reserva. Intenta de nuevo.');
      return;
    }
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-background px-4 text-center">
        ...
        <p className="mb-3 text-muted-foreground">
          {soloEfectivo ? 'Tu reserva fue registrada. Paga en efectivo el día del partido.' : 'Tu comprobante fue recibido correctamente.'}
        </p>
        <div className="mb-6 flex items-center gap-2 rounded-full bg-yellow-500/10 border border-yellow-500/20 px-4 py-2">
          ...
          <p className="text-sm text-yellow-700 dark:text-yellow-400 font-medium">
            {soloEfectivo ? 'El admin confirmará tu reserva en breve' : 'El admin verificará tu pago en breve'}
          </p>
        </div>
        {esInvitado && emailInvitado ? (
        ...
        <Card className="mb-8 w-full max-w-sm border-border p-5 text-left">
          <div className="space-y-3 text-sm">
            <div className="flex items-center gap-2"><ImageIcon className="h-4 w-4 shrink-0 text-primary" /><span className="font-medium text-foreground">{cancha.name}</span></div>
            <div className="flex items-center gap-2"><Calendar className="h-4 w-4 shrink-0 text-primary" /><span className="capitalize text-foreground">{fechaLabel}</span></div>
            <div className="flex items-center gap-2"><Clock className="h-4 w-4 shrink-0 text-primary" /><span className="text-foreground">{hora} - {horaFin}</span></div>
            <Separator />
            <div className="flex items-center justify-between">
              <span className="text-muted-foreground">{soloEfectivo ? 'Monto a pagar en cancha' : 'Total pagado'}</span>
              <span className="font-bold text-primary">S/ {total}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-muted-foreground">Estado</span>
              <Badge className="bg-yellow-500/10 text-yellow-600 border-yellow-500/20" variant="outline">Pendiente de confirmación</Badge>
            </div>
          </div>
        </Card>
        <div className="flex w-full max-w-sm flex-col gap-3">
          ...
          {!esInvitado && <Button size="lg" onClick={() => router.push('/mis-reservas')}>Ver mis reservas</Button>}
          <Button variant={esInvitado ? 'default' : 'outline'} size="lg" onClick={() => router.push('/')}>Volver al inicio</Button>
        </div>
      </div>
    );
  }
