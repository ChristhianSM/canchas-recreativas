...
function CanchasContent() {
  ...
  useEffect(() => {
    ...
    if (
      ubicacionParam &&
      ubicacionParam !== "Mi ubicación" &&
      ubicacionParam !== "Piura, Perú"
    ) {
      // Buscar coincidencia exacta o parcial en los distritos de las canchas
      const distritoEncontrado = allDistricts.find(
        (d) =>
  ...
  const [searchStep, setSearchStep] = useState<"ubicacion" | "fecha" | "hora">(
    "ubicacion",
  );
  // Valores temporales dentro del modal (se aplican solo al buscar)
  const [modalDate, setModalDate] = useState<Date>(new Date());
  const [modalCalendarMonth, setModalCalendarMonth] = useState<Date>(
    new Date(),
  );
  ...
  const [activeField, setActiveField] = useState<
    "ubicacion" | "fecha" | "hora" | null
  >(null);
  const [desktopUbicacion, setDesktopUbicacion] = useState(
    searchParams.get("ubicacion") || "",
  );
  ...
  const [desktopHora, setDesktopHora] = useState(
  ...
  // Ocultar cancha cuando sus horarios filtrados quedan bloqueados (Path 2)
  const handleCanchaOcupada = (canchaId: string) => {
  ...
  const openSearchModal = (
    step: "ubicacion" | "fecha" | "hora" = "ubicacion",
  ) => {
  ...
  const handleModalSearch = () => {
    ...
    const ubicacionFinal =
      modalUbicacion || searchParams.get("ubicacion") || "Piura, Perú";
        ...
        <div className="fixed inset-0 z-50 bg-white flex flex-col">
                ...
                <div className="max-w-xs mx-auto">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="text-sm font-semibold capitalize">
                      {modalCalendarMonth.toLocaleDateString("es-PE", {
                        month: "long",
                        year: "numeric",
                      })}
                    </span>
            ...
            {searchStep === "hora" && (
              <div>
                ...
                <p className="text-xs text-gray-500 mb-4 capitalize">
                  {modalDate.toLocaleDateString("es-PE", {
                    weekday: "long",
                    day: "numeric",
                    month: "long",
                  })}
                ...
                {modalAvailableHours.length > 0 ? (
                  <div className="grid grid-cols-3 gap-2">
                    {modalAvailableHours.map((time) => {
                      const isToday =
                        getLocalDateString(modalDate) === getLocalDateString();
                  ...
                  <div className="text-center py-8">
                    ...
                    <button
                      onClick={() => setSearchStep("fecha")}
                      className="mt-3 text-xs font-semibold text-[#16a34a] underline"
                    >
                      Cambiar fecha
                    </button>
                  </div>
          ...
          <div className="border-t border-gray-100 px-4 py-3 bg-white">
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 bg-[#16a34a] hover:bg-[#15803d] text-white font-bold py-3 rounded-xl transition-colors text-sm"
            >
                  ...
                  <p className="text-sm font-medium text-gray-800">
                    {desktopFecha
                      ? getLocalDateString(desktopFecha) ===
                        getLocalDateString()
                        ? "Hoy, " +
                          desktopFecha.toLocaleDateString("es-PE", {
                            day: "numeric",
                            month: "short",
                          })
                        : desktopFecha.toLocaleDateString("es-PE", {
                  ...
                  <>
                    ...
                    <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-80 bg-white rounded-2xl shadow-2xl border border-gray-100 z-50 p-4">
                      <div className="flex items-center justify-between mb-4">
                        <button
                          onClick={() => {
                            const d = new Date(desktopFecha || new Date());
                            d.setMonth(d.getMonth() - 1);
                            setDesktopFecha(d);
                          }}
                          className="p-1.5 rounded-full hover:bg-gray-100"
                        ...
                        <span className="text-sm font-semibold capitalize">
                          {(desktopFecha || new Date()).toLocaleDateString(
                            "es-PE",
